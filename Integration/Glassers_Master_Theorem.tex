...
\begin{lemma}[Cauchy-Schlömilch Transformation]
...
\begin{theorem}[Glasser's Master Theorem]\label{glassers_master_theorem}
    Suppose $a_1<a_2<\cdots<a_n$, $c_1,c_2,\ldots,c_n>0$ and $b\in\R$. Define $f:\R\exc\{a_1,a_2,\ldots,a_n\}$ as follows:
    \begin{equation*}
...
\end{theorem}
\begin{proof}
    Define $a_0=-\infty$ and $a_{n+1}=\infty$, and for each $k\in\{0,1,\ldots,n\}$, define $I_k=\qty(a_k,a_{k+1})$. Note that for all $x\in\R\exc\{a_1,a_2,\ldots,a_n\}$, we have:
    \begin{equation*}
        f'(x)
