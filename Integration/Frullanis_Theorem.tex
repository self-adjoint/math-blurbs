...
\begin{theorem}\label{frullanis_theorem_generalized_limit}
...
\begin{theorem}
    ...
    \begin{equation*}
        \int_1^\infty \frac{g(u^b)-g(u^a)}{u\log(u)}\du=\qty(L-g(1))\log(\frac{b}{a})
    \end{equation*}
    Sub $u=e^{-x}$ and $g(u)=f(-\log(u))$, $-\frac{1}{x}\int_1^{e^{-x}} f(v)/v\,dv=L$
    \begin{equation*}
        \int_0^1 \frac{g(u^b)-g(u^a)}{u\log(u)}\du=\qty(L-g(1))\log(\frac{b}{a})
    \end{equation*}
\end{theorem}

\begin{corollary}
    Suppose $f\in C[1,\infty)$, $a,b>0$ and $\lim_{x\to\infty} \frac{1}{\log(x)}\int_1^x \frac{f(y)}{y}\dy=L$ exists and is finite. Then:
    \begin{equation*}
        \int_1^\infty \frac{f(x^b)-f(x^a)}{x\log(x)}\dx=\qty(L-f(1))\log(\frac{b}{a})
    \end{equation*}
\end{corollary}
\begin{proof}
    Define $g:[0,\infty)\to\R$ by $g(u)=f(e^u)$. Then $g\in C[0,\infty)$. Substituting $u=\log(x)$ into the integral yields:
    \begin{equation*}
        \int_1^\infty \frac{f(x^b)-f(x^a)}{x\log(x)}\dx
        = \int_0^\infty \frac{f\qty(e^{bu})-f\qty(e^{au})}{u}\du
        = \int_0^\infty \frac{g(bu)-g(au)}{u}\du
    \end{equation*}
    Similarly, substituting $u=\log(x)$ and $v=\log(y)$ into the limit condition yields:
    \begin{equation*}
        \frac{1}{\log(x)}\int_1^x \frac{f(y)}{y}\dy
        = \frac{1}{u}\int_1^{e^u} \frac{f(y)}{y}\dy
        = \frac{1}{u}\int_0^u f\qty(e^v)\,dv
        = \frac{1}{u}\int_0^u g(v)\,dv
    \end{equation*}
    Thus $\lim_{u\to\infty} \frac{1}{u}\int_0^u g(v)\,dv=\lim_{x\to\infty} \frac{1}{\log(x)}\int_1^x \frac{f(y)}{y}\dy=L$. Applying \Cref{frullanis_theorem_generalized_limit} to $g$ yields:
    \begin{equation*}
        r
    \end{equation*}
\end{proof}

Suppose $f(x)=\sin(\log(x))$.
\begin{align*}
    \int_1^x \sin
\end{align*}

Suppose $\lim_{x\to\infty} \frac{1}{x}\int_0^x f(y)\dy=L$. Substituting
\begin{align*}
